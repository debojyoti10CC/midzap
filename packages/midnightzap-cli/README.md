...
```
npx @midnightzap/cli add <recipe> [dir]   apply a recipe (default dir: .)
npx @midnightzap/cli list                 list recipes
npx @midnightzap/cli doctor [dir]         check an integrated project
...
5. **`vite.config.*`** — marks the `@midnight-ntwrk` WASM runtime external
   so the app builds.
6. **`package.json`** — adds `@midnightzap/sdk`.
...
`VITE_MZ_LIVE=1`. See
[GO_LIVE.md](https://github.com/midnightzap/midnightzap/blob/main/docs/GO_LIVE.md).
